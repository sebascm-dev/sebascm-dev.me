...
| `ActivityCalendar` | client | 12m: week-column heatmap with weekday and month labels, legend "Menos … Más"; 30d/90d: one vertical bar per day scaled to the busiest day, with a max/0 scale and Monday (30d) or month (90d) axis labels, no color legend; 7d: seven columns with day labels. Each cell is focusable and shows the same tooltip on hover and focus (value first, then the date). A "Ver como tabla" toggle renders an accessible table of active days |
